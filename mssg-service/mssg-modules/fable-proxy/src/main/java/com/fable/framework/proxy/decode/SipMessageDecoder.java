package com.fable.framework.proxy.decode;
...
public final class SipMessageDecoder extends MessageToMessageDecoder<DatagramPacket> {
    ...
    public SipMessageDecoder(final Clock clock) {
        this.clock = clock;
    }
    ...
    protected void decode(final ChannelHandlerContext ctx, final DatagramPacket msg, final List<Object> out)
        ...
        String sipStr = new String(b, "gb2312");
        if (sipStr.contains("<?xml version=\"1.0\"?>") && sipStr.contains("Application/SDP")) {
            sipStr = sipStr.replace("SDP", "MANSCDP+xml");
        }
        final byte[] sipStrBytes = sipStr.getBytes("gb2312");
        final Buffer buffer = Buffers.wrap(sipStrBytes);
